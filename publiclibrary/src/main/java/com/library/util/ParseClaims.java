...
public class ParseClaims implements JwtParser {
    ...
    public void setObserverToJson(ObserverToJson mObserverToJson) {
        this.mObserverToJson = mObserverToJson;
    }
    ...
    public Jwt parse(String jwt) throws ExpiredJwtException, MalformedJwtException, SignatureException {
        ...
        if (base64UrlEncodedDigest != null) { //it is signed - validate the signature
            ...
            if (key != null && keyBytes != null) {
                ...
                throw new IllegalStateException("A signing key resolver and " + object + " cannot both be specified. " +
                        "Choose either.");
            }
            ...
            if (key == null) { //fall back to keyBytes
                ...
                if (!Objects.isEmpty(keyBytes)) {
                    ...
                    Assert.isTrue(algorithm.isHmac(),
                            "Key bytes can only be specified for HMAC signatures. Please specify a PublicKey or " +
                                    "PrivateKey instance.");
        ...
        if (base64UrlEncodedDigest != null) {
            DefaultJws<Object> objectDefaultJws = new DefaultJws<>((JwsHeader) header, body, base64UrlEncodedDigest);
            try {
                if (mObserverToJson != null) {
                    mObserverToJson.toClaims((Claims)objectDefaultJws.getBody());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            return objectDefaultJws;
        } else {
            DefaultJwt<Object> objectDefaultJwt = new DefaultJwt<>(header, body);
            try {
                if (mObserverToJson != null) {
                    mObserverToJson.toClaims((Claims)objectDefaultJwt.getBody());
                }
            } catch (Exception e) {
            }
            return objectDefaultJwt;
        }
    }
