...
public class WheelView extends View {
    ...
    private void doScroll(int delta) {
        ...
        int count = (int) (scrollingOffset / itemHeight);

        int pos = initPosition - count;
        int itemCount = adapter.getItemsCount();

        int fixPos = (int) (scrollingOffset % itemHeight);
        if (Math.abs(fixPos) <= itemHeight / 2) {
            fixPos = 0;
        }
        if (isLoop && itemCount > 0) {
            if (fixPos > 0) {
                pos--;
                count++;
            } else if (fixPos < 0) {
            ...
            if (pos < 0) {
                ...
                count = initPosition - itemCount + 1;
                pos = itemCount - 1;
        ...
        // update offset
        scrollingOffset = (int) (offset - count * itemHeight);
