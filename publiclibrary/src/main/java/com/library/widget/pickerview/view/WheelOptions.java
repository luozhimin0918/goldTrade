...
public class WheelOptions<T> {
    ...
    private OnWheelChangedListener wheelListener_option1;
    private OnWheelChangedListener wheelListener_option2;
...
//                    opt3 = opt3 >= mOptions3Items.get(opt1Select).get(index).size() - 1 ? mOptions3Items.get(opt1Select).get(index).size
//                            () - 1 : opt3;
//                    */
...
//            }
//        };
        wheelListener_option1 = new OnWheelChangedListener() {
            @Override
            public void onChanged(WheelView wheel, int oldValue, int newValue) {
                ...
                if (mOptions2Items != null) {
                    ...
                    */
                    ...
                    wv_option2.setAdapter(new ArrayWheelAdapter(mOptions2Items.get(newValue)));
                    wv_option2.setCurrentItem(opt2Select);
                }
                if (mOptions3Items != null) {
                    wheelListener_option2.onChanged(wv_option2, 0, opt2Select);
                }
            }
        };

        wheelListener_option2 = new OnWheelChangedListener() {
            @Override
            public void onChanged(WheelView wheel, int oldValue, int newValue) {
                //新opt3位置,默认0
                int opt3 = 0;
                if (mOptions3Items != null) {
                    int opt1Select = wv_option1.getCurrentItem();
                    opt1Select = opt1Select >= mOptions3Items.size() - 1 ? mOptions3Items.size() - 1 : opt1Select;
                    newValue = newValue >= mOptions2Items.get(opt1Select).size() - 1 ? mOptions2Items.get(opt1Select).size() - 1 : newValue;
                    /*
                    //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    opt3 = wv_option3.getCurrentItem();//上一个opt3的选中位置
                    opt3 = opt3 >= mOptions3Items.get(opt1Select).get(newValue).size() - 1 ? mOptions3Items.get(opt1Select).get(newValue)
                    .size
                            () - 1 : opt3;
                    */
                    wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items
                            .get(wv_option1.getCurrentItem()).get(newValue)));
                    wv_option3.setCurrentItem(opt3);
...
//            wv_option2.setOnItemSelectedListener(wheelListener_option2);
    ...
    public int[] getCurrentItems() {
        ...
        if (mOptions2Items != null && mOptions2Items.size() > 0) {//非空判断
            currentItems[1] = wv_option2.getCurrentItem() > (mOptions2Items.get(currentItems[0]).size() - 1) ? 0 : wv_option2
                    .getCurrentItem();
        } else {
        ...
        if (mOptions3Items != null && mOptions3Items.size() > 0) {//非空判断
            currentItems[2] = wv_option3.getCurrentItem() > (mOptions3Items.get(currentItems[0]).get(currentItems[1]).size() - 1) ? 0 :
